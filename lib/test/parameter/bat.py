...
def _resolve_test_checkpoint(prj_dir, save_dir, yaml_name, epoch, network_path=None):
    ...
    1) CTETRACK_CHECKPOINT, BAT_CHECKPOINT, or TEST_CHECKPOINT env
    2) models/CTETrack_{yaml}.pth(.tar) or legacy BAT_{yaml}.pth(.tar)
    ...
    """
    env_ckpt = (
        os.environ.get("CTETRACK_CHECKPOINT")
    ...
    raise FileNotFoundError(
        ...
        + tried
        + "\nSet CTETRACK_CHECKPOINT=/path/to/checkpoint.pth.tar or place weights under models/."
    )
