...
class BATActor(BaseActor):
    ...
    def _compute_moe_aux_temporal_loss(
        ...
        details["Loss/moe_feature"] = total.detach()
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        if self._moe_router_training_active:
            ...
            if isinstance(quality_logit_layers, dict):
                for layer_idx, quality_logits in quality_logit_layers.items():
                    if not torch.is_tensor(quality_logits):
                        continue
                    if quality_logits.ndim != 2 or quality_logits.shape[-1] != 2:
                        raise ValueError(
                            f"Router logit shape mismatch at layer {layer_idx}: "
                            f"logits={tuple(quality_logits.shape)}"
                        )
                    route_probability = F.softmax(quality_logits, dim=-1)
                    hard_usage = F.one_hot(
                        route_probability.detach().argmax(dim=-1),
                        num_classes=2,
                    ).to(route_probability.dtype).mean(dim=0)
                    mean_probability = route_probability.mean(dim=0)
                    balance_losses.append(
                        2.0 * (hard_usage * mean_probability).sum()
                    )
            ...
            router_status = {
                "Loss/moe_balance": router_balance_loss.detach(),
                "Loss/moe_balance_weighted": router_loss.detach(),
            }
        ...
        if return_status:
            ...
            status = {"Loss/total": loss.item(),
                      ...
                      "IoU": mean_iou.item()}
            if aux_moe is not None:
                status["Loss/moe_feature"] = aux_moe.item()
                status["Loss/moe_feature_weighted"] = aux_moe_weight * aux_moe.item()
