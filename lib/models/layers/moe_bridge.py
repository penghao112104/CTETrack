...
from lib.models.layers.mamba_expert_current import CurrentSelfAttentionExpert
from lib.models.layers.mamba_expert_past_future import PastFutureMambaExpert
...
class TemporalQualityRouter(nn.Module):
    ...
    def forward(self, search_rgb, search_tir):
        if search_rgb.shape != search_tir.shape:
            raise ValueError(
                "RGB/TIR router inputs must have the same [B, N, C] shape."
            )
        rgb_summary = self._summarize_modality(search_rgb)
        tir_summary = self._summarize_modality(search_tir)
        return self.quality_head(torch.cat([rgb_summary, tir_summary], dim=-1))


class MoEBridge(nn.Module):
    """Route post-backbone expert features in the joint token space."""
    ...
    def __init__(
        ...
        self.router = TemporalQualityRouter(
            dim=self.inner_dim,
            hidden_ratio=router_hidden_ratio,
            dropout=dropout,
        )
        self.router_hidden_dim = self.router.hidden_dim
    ...
    def reset_router_heads(self):
        ...
        nn.init.zeros_(self.router.quality_head[-1].bias)
    ...
    def route_candidate_pair(self, rgb_bundle, tir_bundle):
        """Route from current Search features; candidates are supervision only."""
        base_rgb = rgb_bundle["base_current_search"]
        base_tir = tir_bundle["base_current_search"]
        router_trainable = self.training and any(
            parameter.requires_grad for parameter in self.router.parameters()
        )
        if router_trainable:
            logits = self.router(base_rgb, base_tir)
        else:
            with torch.no_grad():
                logits = self.router(base_rgb, base_tir)
        ...
        past_available = bool(rgb_bundle["past_available"] and tir_bundle["past_available"])
    ...
    def apply_candidate_route(self, bundle, route, return_expert_hist_ctx: bool = False):
        """Apply the global hard route and update this stream's temporal caches."""
        hard = route["hard"]
