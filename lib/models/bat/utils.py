...
def resolve_encoder_layer_indices(indices, depth):
    """
    Map YAML/config layer indices to valid 0..depth-1 indices.
    -1 means the last encoder block (depth - 1).
    """
    out = set()
    for idx in indices:
        i = int(idx)
...
def resolve_moe_layer_indices(moe_cfg, depth):
    """
    Resolve the MoE insertion layer.

    MODEL.MOE.INSERT_LAYER is user-facing and 1-based:
    4 means "after the 4th encoder block". -1 means the last block.
    """
