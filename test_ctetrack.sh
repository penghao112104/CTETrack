...
CHECKPOINT=${CTETRACK_CHECKPOINT:-${BAT_CHECKPOINT:-}}
if [ $# -gt 0 ] && [[ "$1" != --* ]]; then
  CHECKPOINT=$1
  shift
fi
...
if [ -z "$SEQ_HOME" ]; then
  ...
  echo "Usage: SEQ_HOME=/path/to/dataset bash test_ctetrack.sh /path/to/checkpoint.pth.tar" >&2
  exit 1
fi
...
CTETRACK_CHECKPOINT=$(cd "$(dirname "$CHECKPOINT")" && pwd)/$(basename "$CHECKPOINT")
mkdir -p "$RESULT_ROOT"
RGBT_RESULT_ROOT=$(cd "$RESULT_ROOT" && pwd)
export CTETRACK_CHECKPOINT
export BAT_CHECKPOINT="$CTETRACK_CHECKPOINT"
export RGBT_RESULT_ROOT
export RGBT_SAVE_NAME=${RGBT_SAVE_NAME:-CTETrack}
...
echo "[test] checkpoint: $CTETRACK_CHECKPOINT"
